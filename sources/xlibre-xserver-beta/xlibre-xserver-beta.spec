...
%global upstream_name xlibre-xserver
...
Source0:    https://github.com/X11Libre/%{reponame}/archive/refs/tags/%{upstream_name}-%{version}.tar.gz
...
%description
XLibre X server beta. X11Libre/xserver on GitHub. A maintained fork of X.Org X11.
...
Requires:       pkgconfig
Provides:       xlibre-xserver-devel = %{version}-%{release}
...
Conflicts:      xlibre-xserver-source
BuildArch:      noarch
...
%prep
%setup -q -n %{reponame}-%{upstream_name}-%{version}
...
* Fri Jun 19 2026 Anders da Silva Rytter Hansen <andersrh@users.noreply.github.com> - 25.1.7-1
- Upgrade XLibre to version 25.1.7
...
- Fix URL tag
- Use %%setup -q
- Readd xf86CheckRealOption used by DPIScaleFactor hack in input-libinput driver

* Tue Aug 12 2025 Kevin Kofler <Kevin@tigcc.ticalc.org> - 25.0.0.8-1
- Switch upstream to the X11Libre GitHub project
- Rename package from xorg-x11-server to xlibre-xserver with Obsoletes/Provides
- Drop i965 va_gl patch, fixed upstream with different code
- Rebase Nouveau modesetting patch
- Do not ship driver rebuild script until it is updated
- Do not use %%autosetup
- Drop obsolete (insecure DES) secure_rpc option (was set to false anyway)
- Handle the new ABI-versioned module directory
